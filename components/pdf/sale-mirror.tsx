    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text>{company.document} • {company.email}</Text>
          <Text>{company.address}, {company.number} - {company.neighborhood}</Text>
          <Text>{company.city} - {company.state} | CEP: {company.zip_code}</Text>
        </View>

        <View style={styles.section}>
          <Text style={styles.bold}>Cliente:</Text>
          <Text>{customer.name}</Text>
          <Text>{customer.document} • {customer.email}</Text>
          <Text>{customer.address}, {customer.number} - {customer.neighborhood}</Text>
          <Text>{customer.city} - {customer.state} | CEP: {customer.zip_code}</Text>
        </View>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.bold}>
            R$ {items.reduce((acc: number, item: any) => acc + item.quantity * item.unit_price, 0).toFixed(2)}
          </Text>
        </View>
      </Page>
    </Document>
