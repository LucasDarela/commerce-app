...
export default function DataFinancialTable() {
  ...
  const fetchAll = async () => {
    ...
    const parsedFinancials = financialSchema
      .array()
      .safeParse(financialRes.data);
    if (parsedFinancials.success) {
      const financialsWithSource: FinancialRecord[] = parsedFinancials.data.map(
        (f) => ({
          ...f,
          source: "financial",
        }),
      ...
      console.error(
        "Erro ao validar financials com Zod:",
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <a href={link} target="_blank" rel="noopener noreferrer">
                  ...
                  >
                    <path
                      d="M19.11 17.205... (svg completo) ..."
                      fillRule="evenodd"
                    />
                  </svg>
                </a>
              </TooltipTrigger>
              <TooltipContent>Send message via WhatsApp</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        );
      },
