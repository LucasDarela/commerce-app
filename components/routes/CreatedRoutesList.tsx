...
export function CreatedRoutesList({
  onEditRoute,
}: {
  ...
  const [routeToDelete, setRouteToDelete] = useState<DeliveryRoute | null>(
    null,
  );
    ...
    <div className="flex flex-col gap-4 p-4 mt-8 border-t border-muted w-full">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {routes.map((route) => {
            const driverInfo = teamMembers.find(
              (m) => m.id === route.driver_id,
            );
            const driverName =
                ...
                <CardContent className="p-0">
                  <div className="max-h-[250px] overflow-y-auto p-4 space-y-3">
                    <p className="text-xs text-muted-foreground font-semibold mb-2 uppercase">
                      Sequência ({route.stops?.length || 0} paradas)
                    </p>
                    {route.stops &&
                      route.stops.map((stop: any, idx: number) => (
                        <div key={idx} className="flex gap-3 text-sm">
                          <div className="flex flex-col items-center">
                            <div
                              className={`w-2.5 h-2.5 rounded-full mt-1 ${stop.isDepot ? "bg-black dark:bg-white" : "bg-primary"}`}
                            />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>#{routeToDelete?.route_number}</strong> será excluída do
              sistema e os pedidos associados voltarão ao status "Pendente" para
              que possam ser adicionados em outra rota.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
