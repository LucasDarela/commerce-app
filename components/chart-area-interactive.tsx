...
const chartConfig = {
  ...
  mobile: { label: "Financial Records", color: "var(--primary)" },
} satisfies ChartConfig;
...
export function ChartAreaInteractive() {
  ...
  React.useEffect(() => {
    ...
    async function fetchData() {
      ...
      const startDate = new Date(today);
      startDate.setDate(today.getDate() - 90);
      const startIso = startDate.toISOString(); // ✅ usa ISO completo para timestamps
      ...
      // ✅ escolha 1 campo para filtrar (recomendo issue_date)
      ...
      for (const record of (financials as any[]) ?? []) {
        const date = new Date(record.issue_date).toISOString().split("T")[0];
        dateMap[date] = dateMap[date] || { desktop: 0, mobile: 0 };
        dateMap[date].mobile += Number(record.amount || 0);
      }
