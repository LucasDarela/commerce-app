...
export function EquipmentLoansHistoryReport({
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        // 🔐 garante que tem usuário autenticado
        const {
          data: { user },
          error: userError,
        ...
        // 🔥 executa queries em paralelo (performance)
        ...
        if (endDate) {
          loanQuery.lte("loan_date", endDate);
          returnQuery.lte("return_date", endDate);
        }

        if (customerId) {
          loanQuery.eq("customer_id", customerId);
          returnQuery.eq("customer_id", customerId);
        }

        const [
          { data: loanRows, error: loanErr },
          { data: retRows, error: retErr },
        ] = await Promise.all([loanQuery, returnQuery]);
