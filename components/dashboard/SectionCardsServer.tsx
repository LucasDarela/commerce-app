...
export function SectionCards() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const totalReceber = ordersCurrent.data?.reduce((sum, o) => sum + Number(o.total), 0) || 0
      const totalReceberLast = ordersLast.data?.reduce((sum, o) => sum + Number(o.total), 0) || 0
      const totalPagar = inputsCurrent.data?.reduce((sum, i) => sum + Number(i.amount), 0) || 0
      ...
      const crescimento = receitaAnterior > 0 ? ((receitaAtual - receitaAnterior) / receitaAnterior) * 100 : 100
