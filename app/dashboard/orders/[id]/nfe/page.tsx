...
export default function EmitNfePage() {
  ...
  const handleEmit = async () => {
    ...
    if (!operacaoFiscal) return toast.error("Selecione um tipo de operação fiscal")
    ...
    try {
      ...
      toast.success(`NF-e emitida com sucesso! Número: ${result.numero || "--"}`);
    ...
    <div className="p-6 space-y-4 max-w-3xl mx-auto">
      ...
      <div className="border p-4 rounded-md">
        ...
        <ul className="text-sm mt-2">
          {products.map((p, i) => (
            <li key={i}>
            {p.name} - Qtd: {p.quantity} - R$: {p.price} - CFOP: {selectedOperation && operations.find(o => o.id === selectedOperation)?.cfop || "--"}
            </li>
          ))}
        </ul>
      </div>

      <Button className="mt-4" onClick={handleEmit} disabled={!selectedOperation}>
        Emitir NF-e
      </Button>
    </div>
  )
