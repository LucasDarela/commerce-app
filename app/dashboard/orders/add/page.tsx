...
export default function AddOrder() {
  ...
  function gerarNumeroNota(tipo: string): string {
    const timestamp = Date.now().toString().slice(-6) // últimos 6 dígitos
    return tipo === "fiscal" ? `${timestamp}` : `${timestamp}`
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 rounded-lg shadow-md">
      ...
      <Card className="mb-6">
        ...
        <h2 className="text-xl font-bold mb-4">Selecione a Nota</h2>
          <div className="flex flex-col-2 w-full h-auto gap-4">

          <Select
        onValueChange={(value) => {
          const numeroNotaGerado = gerarNumeroNota(value)
          setVenda({ ...venda, tipoDocumento: value, numeroNota: numeroNotaGerado })
        }}
      >
        ...
        value={venda?.numeroNota}
        onChange={(e) =>
          setVenda({ ...venda!, numeroNota: e.target.value })
        }
      />
          </div>
          ...
          <div className="grid grid-cols-5 gap-4">
            <div className="col-span-4 relative">
              ...
              {mostrarClientes && clientesFiltrados.length > 0 && (
                <div className="absolute z-10 mt-1 w-full  border border-gray-300 rounded-md shadow-md max-h-40 overflow-y-auto">
