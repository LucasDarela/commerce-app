...
import { Check, ChevronRight, Smartphone, Clock, Users } from "lucide-react";
...
export default function BillingPage() {
    ...
    async ({ showLoading = false }: { showLoading?: boolean } = {}) => {
      ...
      try {
        ...
        const queries = await Promise.all([
          ...
          supabase
            .from("companies")
            .select("billing_exempt, mobile_offline_enabled")
            .eq("id", companyId)
            .maybeSingle(),
        ]);
  ...
  const displayedPlans = [
    plans.find(
      (p) => p.name.includes("Essential") && p.interval === currentInterval,
    ),
    plans.find((p) => p.name.includes("Pro") && p.interval === currentInterval),
    plans.find(
      (p) => p.name.includes("Enterprise") && p.interval === currentInterval,
    ),
  ...
  const planDetails: Record<string, { note: string; features: string[] }> = {
    Essential: {
      note: isYearly
        ? "Economia de 10% no anual."
        : "Ideal para organizar sua operação.",
      features: [
        "Gestão de pedidos, financeiro e estoque",
    ...
    Pro: {
      note: isYearly
        ? "10% de desconto no plano total."
        : "O plano mais completo para gestão.",
      features: [
        "Tudo do Essential",
  ...
  const currentPlan = plans.find(
    (p) => p.stripe_price_id === subscriptionData?.price_id,
  );
  const isSubscribed =
  ...
  const getExtraUserPrice = () => {
    const planName =
      currentPlan?.name.split(" ")[0] || displayedPlans[0]?.name.split(" ")[0];
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm overflow-hidden relative">
          ...
          <div className="space-y-2 text-center md:text-left relative z-10">
            <h2 className="text-xl font-bold flex items-center justify-center md:justify-start gap-2">
              <span
                className={`w-2.5 h-2.5 rounded-full animate-pulse ${subscriptionData?.status === "trialing" ? "bg-blue-500" : "bg-emerald-500"}`}
              />
              Sua assinatura {currentPlan?.name.split(" ")[0]} está{" "}
              {subscriptionData?.status === "trialing"
                ? "em Período de Teste"
                : "Ativa"}
            </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 relative z-10 w-full md:w-auto">
            ...
            <Button
              ...
              disabled={submitting}
              className="h-11 px-6 font-semibold transition-all hover:bg-primary hover:text-primary-foreground"
            >
              Gerenciar no Stripe
            </Button>
          </div>
      ...
      <div className="text-center space-y-4">
        ...
        <div className="flex flex-col items-center gap-4 pt-4">
          <div className="flex items-center justify-center gap-4">
            <span
              className={
                !isYearly ? "font-semibold" : "text-muted-foreground text-sm"
              }
            >
              Mensal
            </span>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {displayedPlans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative flex flex-col p-8 bg-card rounded-3xl border shadow-sm transition-all hover:shadow-md ${isCurrent ? "border-emerald-500 ring-1 ring-emerald-500" : name === "Pro" && !isSubscribed ? "border-primary ring-1 ring-primary" : ""}`}
            >
              {name === "Pro" && !isCurrent && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  MAIS VENDIDO
                </span>
              )}
              {isCurrent && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-500 text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  SEU PLANO
                </span>
              )}
              <div className="mb-8">
              ...
              <ul className="flex-1 space-y-4 mb-8 text-sm">
                {details.features.map((feat, i) => (
                  <li key={i} className="flex items-center gap-3">
                    <Check
                      className={`h-4 w-4 ${feat.includes("muito mais") ? "text-primary" : "text-emerald-600"}`}
                    />
                    <span
                      className={
                        feat.includes("muito mais")
              ...
              <Button
                ...
                onClick={() =>
                  !isCurrent && handleCheckout(plan.stripe_price_id)
                }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground leading-tight mt-1">
                  Sincronize entregas sem internet.
                </p>
              </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground leading-tight mt-1">
                  Expanda sua equipe operacional.
                </p>
              </div>
              <div className="text-right">
                <span className="text-base font-bold text-primary italic">
            ...
            <div className="flex items-center gap-4 mt-4 bg-primary/5 p-2 rounded-2xl border border-primary/10 w-fit">
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setExtraUsers(Math.max(0, extraUsers - 1));
                }}
              >
                -
              </Button>
            ...
            <p className="text-[10px] text-muted-foreground mt-3 italic font-medium">
              * Seu plano atual inclui{" "}
              {currentPlan?.name.includes("Enterprise")
                ? 15
                : currentPlan?.name.includes("Pro")
                  ? 5
                  : 2}{" "}
              usuários base.
            </p>
