...
export default function IntegrationsPage() {
  ...
  const [integration, setIntegration] = useState<null | { provider: string; access_token: string }>(null)
      ...
      {integration && integration.access_token && !addingNew ? (
        <div className="flex flex-col gap-2 rounded-lg border p-4">
          ...
          <div className="flex items-center gap-2 mt-4">
            <Button
              onClick={async () => {
                const confirm = window.confirm("Deseja realmente desconectar esta integração?")
                if (!confirm) return
        ...
        <div className="rounded-lg border p-4 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => setAddingNew(false)} disabled={saving}>Cancelar</Button>
          </div>
        </div>
      ) : (
        <div className="rounded-lg border p-4">
          ...
          <Button onClick={() => setAddingNew(true)} className="bg-primary">Adicionar Integração</Button>
        </div>
      )}
    </div>
  )
